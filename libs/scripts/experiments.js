...
const handleCommands = (
  ...
  cmdList.forEach((name) => {
    ...
    if (
      ...
      control[name][i] &&
      selectedVariant[name][i]
    ) {
      let targetEls;
      const query = control[name][i].startsWith('.')
        ? control[name][i]
        : `.${control[name][i]}`;
      if (all) {
      ...
      targetEls.forEach((el) => {
        ...
        if (cmd !== 'remove' && content[selectedVariant[name][i]]) {
          COMMANDSHTML[cmd](el, content[selectedVariant[name][i]]);
        } else {
          COMMANDS[cmd](
            el,
            cmd !== 'remove' && createFragment(selectedVariant[name][i])
          );
        }
      });
    }
...
export function parseExperimentConfig(json) {
  ...
  try {
    ...
    experiences.data.forEach((line) => {
      ...
      vns.forEach((vn) => {
        ...
        if (arrayProperties.includes(key)) {
          ...
          if (key === 'pages')
            variants[camelVN][key].push(
              line[vn] ? new URL(line[vn]).pathname : ''
            );
    ...
    config.variantLabels = Object.entries(variants).reduce(
      (labelMap, [variantName, variant]) => {
...
export function isValidConfig(config) {
  if (
    ...
    console.warn(
      'Invalid experiment config. Please review your sheet and parser.'
    );
    return false;
  }
  return true;
...
export async function replaceInner(path, element) {
  ...
  plainPath = plainPath.endsWith('.plain.html')
    ? plainPath
    : `${plainPath}.plain.html`;
  try {
...
export function getConfigForInstantExperiment(experimentId, instantExperiment) {
  ...
  const pages = instantExperiment
    .split(',')
    .map((p) => new URL(p.trim()).pathname);
 ...
 * @returns {object} containing the experiment manifest
 */
export async function getConfigForFullExperiment(
  experiment = '',
...
export async function getConfig(
  ...
  if (instantExperiment) {
    ...
    config = getConfigForInstantExperiment(
      experimentName || 'not defined',
      instantExperiment
    );
  } else {
...
const checkForPageReplacement = async (
  controlPages,
  selectedPages,
  id,
  content
...
const convertToMap = (blockName, control, selectedVariant) => {
  if (!control[blockName]?.length || !selectedVariant[blockName]?.length)
    return;
...
export async function runExperiment(
  ...
  const experiment = await getConfig(
    experimentPath,
    variantLabel,
    manifestData,
    instantExperiment
  ...
  if (
    !experiment.selectedVariant ||
    experiment.selectedVariantName === experiment.controlName
  ) {
    return;
  }
